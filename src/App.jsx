...
import { loadEvents, setEvents } from './store/eventsSlice'
...
export default function App(){
  ...
  const { emit } = useSocket((type,payload)=>{
    if(type==='events') dispatch(setEvents(payload))
    if(type==='chat') chatHandlerRef.current && chatHandlerRef.current(type,payload)
  })

  useEffect(()=>{ dispatch(loadEvents()) },[])

  const [activeEvent,setActiveEvent] = React.useState(null)
  const handleBookConfirm = async (payload)=>{
    try{
      // dispatch booking via slices from pages/components where used
      await fetch('/api/book', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload) })
      setActiveEvent(null)
      alert('Booked')
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto p-4">
        <Routes>
          <Route path="/" element={<Home onBook={setActiveEvent} />} />
          <Route path="/admin" element={<Admin />} />
        </Routes>
        <div className="fixed right-4 bottom-4">
          <ChatBox socketEmit={emit} onEvent={chatHandlerRef} />
        </div>
      </main>
      <BookingModal event={activeEvent} onClose={()=>setActiveEvent(null)} onBook={handleBookConfirm} />
    </div>
  )
}
